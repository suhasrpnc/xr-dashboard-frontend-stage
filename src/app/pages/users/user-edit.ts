...
@Component({
  ...
  imports: [
    ...
    SelectModule, DatePickerModule, ToggleButtonModule, ToggleSwitchModule,
    MultiSelectModule, TextareaModule,
    Toast,
    DialogModule
],
  template: `
    <p-toast></p-toast>
      <div class="max-w-8xl mx-auto p-4 bg-white rounded-lg shadow-md">
        <form class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6"  #userForm="ngForm" (ngSubmit)="onSubmit()" novalidate>
      ...
      <p-dialog header="Confirm Delete" [(visible)]="displayConfirmation" [style]="{ width: '350px' }" [modal]="true">
        ...
        <ng-template pTemplate="footer">
          ...
          <p-button label="Yes" icon="pi pi-check" (click)="deleteUser()" severity="danger" autofocus></p-button>
        </ng-template>
      </p-dialog>
    `
})
export class EditUser implements OnInit {
  ...
  onSubmit() {
    ...
    this.userService.putUserEdit(payloadData).subscribe({
      next: (result: any) => {
        this.messageService.add({
          severity: 'success',
          summary: 'Success',
  ...
  deleteUser() {
    ...
    if (!id) {
      this.messageService.add({
        severity: 'error',
        summary: 'Error',
        detail: 'Cannot delete user. Invalid user ID.'
      });
